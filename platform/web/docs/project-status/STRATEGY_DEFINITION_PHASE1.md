...
`usableForBacktest`/`usableForDemo`, compatible instruments, total version count); rejections are
capped at 10 in human output (same policy as `instrument:catalogue`) with the full curated list
always available via `--json`, including each strategy's complete version history.
...
{ strategyId, strategyVersion, valid, usableForBacktest, usableForDemo,
  ...
  prohibitedFieldsFound, provenance }
```
