...
import { addRecordsForUser } from "@/lib/decision-intelligence/server-decision-history-store";
...
// usePaperTrades()/useBotDecisionLog()/useDecisionHistory()), and the identical risk pipeline runs
// — this time persisting through the service role instead of a browser session. Deliberately kept
// out of src/lib/bot/index.ts (the client-safe barrel): this module (and everything it imports)
// carries "server-only", so re-exporting it from the barrel would make importing @/lib/bot from a
// client component fail the build. Import this file directly by path from server-only call sites.
// See docs/product/MISSION-6-SERVER-ARCHITECTURE-PREPARATION.md and
// docs/product/MISSION-7-DECISION-INTELLIGENCE.md.
export function createServerExecutionContext(
  ...
  return {
    ...
    persistDecisionRecords: (records: DecisionRecord[]) => addRecordsForUser(client, userId, records),
  };
}
