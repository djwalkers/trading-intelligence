...
/**
 * Pure — never mutates `value`, only reads it while building a new string. Recursively sorts
 * OBJECT keys so two documents that differ only in key order canonicalise identically; ARRAY order
 * is preserved as-is, since array order is semantically meaningful (e.g. `supportedInstruments`,
 * `signalExitRules`, `AND`/`OR` child rule order). This is the ONE canonicalisation implementation
 * used both for content hashing (`computeContentHash` below) and for the registry's own
 * identical-vs-conflicting duplicate detection (strategy-definition-registry.ts's `recordsEqual`,
 * which compares content hashes derived from this exact function) — so the two concepts can never
...
/**
 * Deterministic SHA-256 over the canonicalised, VALIDATED strategy document only — never the
 * source file path, absolute directory, load timestamp, filesystem metadata, or any environment
 * value (none of those are part of `document` at all; they live on `ValidatedStrategyRecord`/
 * `StrategyValidationResult` instead, entirely separate from the hashed value). Identical documents
 * hash identically regardless of their source JSON text's own key order; a materially different
 * document (a changed indicator period, a changed rule, a changed status, a reordered array where
 * order is meaningful) hashes differently. This hash is content identity ONLY — it is never, by
 * itself, evidence that a strategy is approved, validated, or trusted for anything.
 */
export function computeContentHash(document: unknown): string {
  return createHash(CONTENT_HASH_ALGORITHM).update(canonicalStringify(document)).digest("hex");
}
...
export interface StrategyValidationResult {
  ...
  /** `contentHash`/`contentHashAlgorithm` are content identity only — a hash proves two documents
   * are (or aren't) byte-for-byte-equivalent-once-canonicalised, never that a strategy is approved
   * or trusted for backtest/demo use (see `usableForBacktest`/`usableForDemo` for that). Never
   * includes `filePath` or any other non-content fact in what it hashes.
   *
   * `loadedAt` is OPERATIONAL provenance, not document content: the UTC ISO-8601 instant this
   * process validated the document, always generated by the caller (see this function's own
   * `loadedAt` parameter) — never read from the document itself (there is no `loadedAt` field in
   * the schema; one appearing in a strategy file is rejected as an unrecognised root field, exactly
   * like any other unknown field). Excluded from `contentHash` and from duplicate-equality
   * comparison (`strategy-definition-registry.ts`'s `recordsEqual` compares `contentHash` alone) and
   * plays no part in identity or latest-version selection. */
  provenance: { filePath: string; contentHash: string; contentHashAlgorithm: typeof CONTENT_HASH_ALGORITHM; loadedAt: string };
}
...
/**
 * Validates one already-parsed JSON value as a Phase 1 strategy definition, cross-checked against
 * ALREADY-LOADED instrument catalogue entries (never re-fetches or re-derives catalogue state
 * itself — see this module's own top-of-file note on reusing, never duplicating, the instrument
 * catalogue). `catalogueEntries` should be the Phase 0 catalogue's own `InstrumentCatalogueEntry[]`
 * (BTC/ETH/SOL) — an instrument absent from this list is entirely unknown to the platform, never
 * merely "unverified".
 *
 * `loadedAt` is the one non-pure input: the UTC ISO-8601 instant to record as this validation's own
 * operational provenance — always supplied by the CALLER (a registry load operation shares one
 * value across every file it processes in that operation; see `loadStrategyDefinitions`'s own doc
 * comment), defaulting to the real current time only for direct/standalone callers. Never read from
 * `raw` itself — the document has no `loadedAt` field, and one appearing there is rejected as an
 * unrecognised root key like any other unknown field. Every OTHER aspect of this function remains
 * pure and content-only: `loadedAt` is attached to `result.provenance` only, strictly after
 * `contentHash` has already been computed from `raw` alone, so it can never influence the hash.
 */
export function validateStrategyDefinition(
  ...
  // Hashes `raw` itself — the same value that becomes `record.document` on success — never
  // `filePath` or anything else outside the document's own content.
  ...
  const result: StrategyValidationResult = {
    ...
    provenance: { filePath, contentHash, contentHashAlgorithm: CONTENT_HASH_ALGORITHM, loadedAt },
  };
